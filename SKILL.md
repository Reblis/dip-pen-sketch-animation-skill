...
7. **Render the animation with the bundled script** (`$SKILL_DIR` = this skill's directory):
   ...
   ```
   Defaults: 30fps · 8s pen phase · 0.75s hold on the finished outline · 5s marker phase · 1.5s end hold · 1600px longest side · pen/marker cursor dot. Knobs: `--fps`, `--ink-seconds`, `--marker-seconds`, `--pause`, `--end-hold`, `--max-size`, `--no-cursor`. The script replays BOTH phases like a human. Ink: skeletonize the linework, trace it into individual pen strokes (continuing straight through crossings, so intersecting hatch lines replay as separate strokes), order by nearest-neighbor pen travel, move a tip along each path revealing the stroke's true width. Marker: narrow chisel streaks grouped into patches; the hand travels nearest-neighbor between patches and pulls back-and-forth strokes inside each one — never a region-wide advancing edge. Phase transitions crossfade so no pixels pop in a single frame.
